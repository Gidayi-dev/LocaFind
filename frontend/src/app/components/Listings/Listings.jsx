...
export default function Listings({ category = "Healthcare" }) {
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        // User denied or error — fall back to Nairobi CBD
        setLocation({ lat: -1.2921, lng: 36.8219 });
        setAreaLabel("Nairobi CBD");
      }
    );
  ...
  // Fetch listings once we have location
  useEffect(() => {
    if (!location) return;
    ...
    async function fetchListings() {
      ...
      try {
        ...
        console.error(err);
        setError("Could not load nearby facilities. Check your connection.");
      } finally {
        setLoading(false);
      }
    }
    ...
    <section className="min-h-screen" style={{ background: "#D9E0E1" }}>
      ...
      <div className="px-4 flex flex-col gap-3 pb-8">
        ...
        {!loading && !error && results.length > 0 &&
          results.map((listing) => (
            <ListingCard key={listing.id} listing={listing} category="healthcare" />
          ))
        }
